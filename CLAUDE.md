...
**Design source of truth:** `.claude/skills/denoisr-design/` (the skill folder). Its `README.md` is the brand spec; `colors_and_type.css` is the token vocabulary. (The legacy `design.md` at the repo root has been removed — it described an older "pure white / The Future font / magenta + orange" direction that's no longer the brand.)
...
3. Page-scoped sheets imported by their own page (e.g. `src/styles/landing-v2.css` under `.editorial-landing-v2`, only loaded by `ProductPageV2.tsx`).
...
- `ProductPageV2.tsx` (`/`) — **the live landing page.** Animated "remove the noise from hiring" narrative. Opens with a **scroll-driven full-screen noise intro** (`.el2-intro`): a dense full-viewport field of feed fragments (cold InMail, recruiter spray, glyph static, notification cards) plus **animated "feature" widgets** — notification spam (`.el2-notif` — bell/message line-SVGs with count badges in an intro-only `--noise-red` that tick up via a `notifTick` interval and pulse) and recruiter-spray message threads (`.el2-msgs` — cold-InMail chat bubbles that cascade in on a CSS loop) — with user **pain points flashing one-by-one** over it; a `position: sticky` stage holds it while a JS scroll handler sets a `--p` (0→1) progress var. To avoid a muddy cross-fade, the intro is **two layers**: the noise fragments + flashing pains fade out early (≈50% scroll) on top of an **opaque paper curtain** (`.el2-intro__curtain`) that stays opaque until ≈55% then lifts last — so the noise fully clears *before* the hero shows, **revealing the hero in place** with no overlap (the hero is the next section, pulled up under the intro via `margin-top: -100vh` + `z-index`, so there is also no blank gap). Then the restructured story — *the noise* (struck-through feed rows) → *the signal* (three primitives) → LinkedIn import → cited research → a count-up stat band → FAQ → CTA. Scoped under `.editorial-landing-v2` in `landing-v2.css` (`el2-` prefix; isolated token copy). Motion is pure CSS + the `src/hooks/useScrollReveal.ts` IntersectionObserver hooks (`useScrollReveal`/`useStaggeredReveal`/`useCountUp`), all with `prefers-reduced-motion` fallbacks (intro degrades to a static noise field with the pain points stacked, no flashing/drift; scroll-clear still works since it is scroll-tied). Shared content + deck live in `src/data/landing.ts` and `src/components/landing/InteractiveDeck.tsx`. App.tsx hides the global Navbar/Footer here. See the `interactive-landing` skill for the motion principles.
...
- `LoginPage.tsx`, `SignupPage.tsx` — editorial auth card with pastel wash, mono labels, pill primary button. Styles in `editorial.css` under `.auth`. `SignupPage` supports a LinkedIn import flow, split into two sequential calls so a Gemini failure doesn't lose the (paid) Apify scrape: `/LoginController/linkedinImport/scrape` scrapes via Apify and returns an `importId`, then `/LoginController/linkedinImport/structure` restructures that job's cached raw data with Gemini and pre-fills the signup form. If the structure call fails, a "Retry" button re-calls `/structure` with the same `importId` (tracked in `importStep`/`importId` state) without re-scraping; if the scrape call fails, the user must resubmit the form from scratch.
...
- `HomePage.tsx` (`/home`) — 3-column dashboard (filters · deck · preview) per the skill's `ui_kits/web/` spec. Drag-swipe, decision stamps that fade in proportional to drag distance, "It's a fit." match overlay, mobile bottom-nav, mobile chip rail for active filters. Job cards show a neutral "Unverified" chip (`.hp-chip--unverified`) next to the company name when the job's `companyVerified` field is false. Scoped CSS in `home.css`, prefix `hp-`.
...
- `JobApplicationsPage.tsx` (`/applications`) — candidate's list of jobs they've swiped right on. Fetches from `/FeedController/jobApplications`. Stats row and status filter pills are computed client-side from the fetched list (no extra requests). Candidates can withdraw an application while it's still active (`new`/`submitted`/`reviewing`/`shortlisted`) via `POST /FeedController/withdrawJobApplication`, two-step confirm in the UI, local state updated in place (no refetch) on success. Scoped CSS in `job-applications.css`, prefix `ja-`.
- `CompanyPage.tsx` (`/company`) — recruiter-facing dashboard: company profile form, job listing management, applicant review with status pipeline (new → reviewing → shortlisted → messaged → hired / passed). Fetches from `CompanyController` endpoints. Shows the company's own `verificationStatus` ("Pending review" / "Verified" / "Rejected", plus `verificationNotes` when rejected) as a `.cp-verifyBadge` pill next to the company name — separate from the candidate-facing "Unverified" chip on `HomePage`. Scoped CSS in `company.css`, prefix `cp-`.
- `AdminCompaniesPage.tsx` (`/admin/companies`) — admin review queue with two tabs. **Companies** tab lists companies with `verification_status='unverified'` from `GET /AdminController/pendingCompanies` and lets an admin Verify/Reject (with optional notes) via `POST /AdminController/reviewCompany`. **Reports** tab lists open `user_reports` from `GET /AdminController/userReports` (each enriched with `reporter`/`reported` person info) and lets an admin mark them Resolved/Dismissed (with optional internal notes) via `POST /AdminController/resolveReport` — this is record-keeping only, it doesn't block the reported user (use the existing block action for that). "Admin" isn't a real role — it's just `people.id` membership in the API's `ADMIN_USER_IDS` env var, checked server-side on every gated request; the page's own access check (`GET /AdminController/isAdmin`, cached via `fetchAndCacheIsAdmin`/`getStoredIsAdmin` in `auth.ts`) is UI convenience only, not the security boundary. The nav link only appears for admins (see `Navbar.tsx`). Scoped CSS in `admin-companies.css`, prefix `adm-`.
- `SettingsPage.tsx` (`/settings`) — tabbed account settings (Security / Notifications / Privacy / Data). The Data tab has two `SettingsController` actions: **export** (`GET /exportData`, downloads the response as a JSON file client-side via a Blob + temporary `<a download>`) and **delete account** (`POST /deleteAccount`, password-gated, two-step confirm UI in `.st-dangerZone`) — on success calls `clearSession()` and navigates to `/`. Scoped CSS in `settings.css`, prefix `st-`.
- Shared components: `Navbar`, `Footer`, `LoadingState` — restyled via `editorial.css`.
