...
- `ProductPage.tsx` — the **previous** landing (kept as rollback; no longer routed). Static `vs the feed` table, how-it-works steps, research, FAQ. Scoped under `.editorial-landing` in `landing.css`. Consumes the same shared `src/data/landing.ts` + `InteractiveDeck`. To roll back, point the `/` route in `App.tsx` at `ProductPage` instead of `ProductPageV2`.
